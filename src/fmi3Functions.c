...
static bool allowedState(ModelInstance *instance, int statesExpected, char *name) {
    ...
    if (instance->status == Fatal) {
        logError(instance, "fmi3%s: Illegal call sequence. The FMU is in a fatal error state; no further calls are allowed for this instance.", name);
        return false;
    }
...
#if MAX_CONTINUOUS_STATES > 0 || MAX_EVENT_INDICATORS > 0
    CALL(configurate(S));
#endif
